...
### 노트북 목록
노트북의 목록을 확인할 수 있습니다. 목록의 노트북을 클릭하면 노트북의 상세 정보를 확인하고 내용을 변경을 할 수 있습니다.

* 노트북 목록 
    - **이름**: 노트북 이름이 표시됩니다. **변경**버튼을 클릭하여 이름을 변경할 수 있습니다.
    ...
    - **상태**: 노트북의 상태가 표시됩니다. 주요 상태는 다음을 참고하시기 바랍니다.
        ...
        - ACTIVE: 인스턴스에 노트북 애플리케이션이 정상적으로 구동 중인 상태입니다.
    ...
    - **작업 > Jupyter 노트북 열기**: **Jupyter 노트북 열기** 버튼을 클릭하면 브라우저의 새 창으로 노트북을 엽니다.
...
4. 변경하려는 인스턴스 타입을 선택하고 확인을 클릭합니다.
    * 인스턴스의 코어 타입과 동일한 코어 타입의 인스턴스 타입으로만 변경이 가능합니다.
...
- **상태**: 학습의 상태가 표시됩니다. 주요 상태는 다음을 참고하시기 바랍니다.
    - CREATE_REQUESTED: 학습 생성이 요청된 상태입니다.
    - CREATE_RESOURCE_IN_PROGRESS: 학습에 필요한 자원을 생성 중인 상태입니다.
    - RUNNING: 학습이 진행 중인 상태입니다.
    - STOPPED: 학습이 사용자의 요청으로 중지된 상태입니다.
    - COMPLETE: 학습이 정상적으로 완료된 상태입니다.
    - STOP_IN_PROGRESS: 학습이 중지 중인 상태입니다.
    - DELETE_IN_PROGRESS: 학습이 삭제 중인 상태입니다.
    - FAILED_TO_TRAIN: 학습 진행 중 실패된 상태입니다. 자세한 실패 정보는 로그 관리가 활성화된 경우, Log & Crash Search 로그를 통해 확인할 수 있습니다.
    - FAILED_TO_CREATE_RESOURCES: 학습 생성이 실패된 상태입니다.
    - FAILED_TO_CLEAR_RESOURCES: 학습에 사용된 자원이 정상적으로 삭제되지 못 한 상태입니다.
    - FAILED_TO_STOP: 학습 중지가 실패된 상태입니다.
    - FAILED_TO_DELETE: 학습 삭제가 실패된 상태입니다.
- **작업**
...
- **상태**: 실험의 상태가 표시됩니다. 주요 상태는 다음을 참고하시기 바랍니다.
    - CREATE_IN_PROGRESS: 실험이 생성중인 상태입니다.
    - CREATE_RESOURCE_IN_PROGRESS: 실험이 생성중인 상태입니다.
    - CREATE_EXPERIMENT_IN_PROGRESS: 실험이 생성중인 상태입니다.
    - ACTIVE: 실험이 정상적으로 생성된 상태입니다.
    - FAILED_TO_CREATE_RESOURCE: 실험 생성이 실패된 상태입니다.
    - FAILED_TO_CREATE_EXPERIMENT: 실험 생성이 실패된 상태입니다.
