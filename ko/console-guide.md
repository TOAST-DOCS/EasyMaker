...
### 노트북 생성
머신러닝 개발을 위한 필수 패키지가 설치되어 있는 Jupyter 노트북
...
- **모델 정보**: 모델의 아티팩트가 저장된 저장소를 입력합니다.
    ...
    - **NHN Cloud NAS**: 모델 아티팩트가 저장된 NAS 경로를 입력합니다.
        - `nas://{NAS ID}:/{path}` 형식으로 디렉터리 경로를 입력합니다.
...
엔드포인트 생성 정보에 대한 자세한 내용은 [엔드포인트](./console-guide/TODO://link) 문서를 참고해주세요.
...
- **상태**: 엔드포인트의 상태입니다. 주요 상태는 아래 표를 참고해주세요.
    ...
    | PART OF STAGE IN PROGRESS | 엔드포인트의 스테이지 일부가 처리 중인 작업이 있는 상태입니다.<br/>자세한 내용은 엔드포인트 스테이지 목록에서 확인하셔야 합니다. |
...
AI EasyMaker의 일부 기능에서 입출력 스토리지로 사용자의 Object Storage를 사용하는 경우,
사용자의 Object Stroage 컨테이너에 AI EasyMaker 시스템 계정에 대한 읽기 또는 쓰기 권한을 허용해주어야 정상적인 기능 동작이 수행될 수 있습니다.

사용자의 Object Stroage 컨테이너에 AI EasyMaker 시스템 계정의 읽기/쓰기 권한을 허용하는 것은 AI EasyMaker 시스템 계정이 사용자의 Object Stroage 컨테이너의 모든 파일에 대해 부여된 권한에 따라 파일을 읽거나 쓸수 있음을 의미합니다. 
이 내용을 반드시 인지하여 필요한 경우에만 접근 권한 추가하고, AI EasyMaker 시스템 계정이 외부로 노출되지 않도록 주의해주시기 바랍니다.
...
| 엔드포인트 | 읽기 | 사용자가 입력한 입력한 모델 아티팩트 경로 | 
...
4. [특정 프로젝트 또는 특정 사용자에게 읽기/쓰기 허용](https://docs.toast.com/ko/Storage/Object%20Storage/ko/acl-guide/#_4) 문서를 참고하여 NHN Cloud Object Storage 콘솔에서 EasyMaker 시스템 계정에 필요한 읽기 및 쓰기 허용 권한을 추가합니다.
