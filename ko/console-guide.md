...
- **학습 리소스 정보**
    ...
    - **torchrun 사용 여부**: Pytorch 프레임워크에서 지원하는 torchrun을 사용할 지 여부를 선택해 주세요. 자세한 사항은 [부록 > 8. torchrun 사용 방법](./console-guide/#8-torchrun-사용-방법)을 참고해 주세요.
    - **노드 당 프로세스 개수**: torchrun을 사용할 경우 하나의 노드에 여러 프로세스를 실행하여 분산 학습이 가능합니다.
- **입력 데이터**
...
- **학습 리소스 정보**
    ...
    - **분산 노드 수**: 분산 학습을 수행할 수를 입력합니다. 알고리즘 코드에서 설정을 통해 분산 학습이 가능하게 할 수 있습니다. 자세한 사항은 [부록 > 6. 프레임워크별 분산 학습 설정](./console-guide/#6)을 참고해 주세요.
    - **torchrun 사용 여부**: Pytorch 프레임워크에서 지원하는 torchrun을 사용할 지 여부를 선택해 주세요. 자세한 사항은 [부록 > 8. torchrun 사용 방법](./console-guide/#8-torchrun-사용-방법)을 참고해 주세요.
    - **노드 당 프로세스 개수**: torchrun을 사용할 경우 하나의 노드에 여러 프로세스를 실행하여 분산 학습이 가능합니다. 프로세스 개수에 따라 메모리 사용량에 영향이 있습니다.
...
- **상태 상세 정보**: `COMPLETE` 상태에 괄호 안에 있는 내용은 상태의 상세 정보입니다. 주요 상세 정보는 아래 표를 참고해 주세요.
...
### 8. torchrun 사용 방법
* Pytorch에서 분산 학습이 가능하도록 코드를 작성하였고, 분산 노드 수와 노드 당 프로세스 개수를 입력하면 torchrun을 이용한 분산 노드 및 멀티 프로세스를 활용한 분산 학습이 이루어집니다.
* 총 프로세스 개수, 모델 크기, 입력 데이터 크기, 배치 사이즈 등의 요소에 의하여 메모리 부족으로 학습 및 하이퍼파라미터 튜닝이 실패할 수 있습니다. 메모리 부족으로 실패하는 경우 아래와 같은 에러 메세지가 남을 수 있습니다. 단, 아래의 메세지가 남는다고 하여 모두 메모리 부족으로 인한 실패는 아닙니다. 메모리 사용량에 따라 적절한 인스턴스 타입을 설정해주세요.
```
exit code : -9 (pid: {pid})
```
