...
### 노트북 인스턴스의 사용자 가상 실행 환경 구성
AI EasyMaker 노트북 인스턴스는 머신 러닝에 필요한 다양한 라이브러리 및 커널이 설치된 Conda 가상 환경을 제공합니다.
이 가상 환경은 노트북을 중지하고 시작할 때 초기화되어 구동되지만, 사용자가 임의의 경로에 설치한 가상 환경 및 외부 라이브러리는 자동으로 초기화되지 않기 때문에 노트북을 중지하고 시작했을 때 유지되지 않습니다.
...
> 유지하려면 [노트북 인스턴스의 사용자 가상 실행 환경 구성](./console-guide/??) 을 참고하여 사용자 가상 환경을 구성해주세요.
...
- **설명**: 학습 설명이 표시됩니다. 상세 화면에서 **변경**버튼을 클릭하여 설명을 변경할 수 있습니다.
...
NHN Cloud Object Storage에 AI EasyMaker 시스템 계정의 권한을 추가하는 작업은 AI EasyMaker 시스템 계정으로 NHN Cloud Object Stroage의 데이터를 읽거나 쓸수 있도록 권한을 허용하는 작업입니다. 
...
다음의 필드를 참고하여 루씬쿼리를 작성하면  Log & Crash Search 서비스 에서 루씬 쿼리로 조회를 할 수 있습니다.
