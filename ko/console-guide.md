...
- **스토리지** 
    ...
    - 필요한 경우 노트북 연결할 **NHN Cloud NAS**를 추가할 수 있습니다.
        ...
        - NAS 경로: `nas://{NAS ID}:/{path}` 형식의 디렉터리 경로를 입력합니다.
...
> AI EasyMaker와 동일한 프로젝트에서 생성된 NAS만 사용 가능합니다.
...
> 연결한 NAS는 삭제되지 않으며 **NHN Cloud NAS**에서 개별 삭제해야 합니다.
...
- **알고리즘 정보**: 학습하려는 알고리즘에 대한 정보를 입력합니다.
    ...
    - **엔트리 포인트**
        - 학습이 시작되는 알고리즘의 엔트리 포인트 파일명을 작성합니다. 엔트리 포인트 파일은 알고리즘 경로에 존재 해야합니다.
...
> **[주의] NHN Cloud NAS를 사용하는 경우**
> 1. AI EasyMaker와 동일한 프로젝트에서 생성된 NAS만 사용 가능합니다.
> 2. 학습이 완료되기 전까지는 입력 데이터를 유지하지 않으면 학습에 실패할 수 있습니다.
...
- **모델 정보**: 모델의 아티팩트가 저장된 저장소를 입력합니다.
    - **NHN Cloud Object Storage**: 모델 아티팩트가 저장된 Object Storage 경로를 입력합니다.
        - `obs://{Object Storage API 엔드포인트}/{containerName}/{path}` 형식으로 디렉터리 경로를 입력합니다.
        - NHN Cloud Object Storage를 이용하는 경우 [부록 > 1. NHN Cloud Object Storage에 AI EasyMaker 시스템 계정 권한 추가](./console-guide//TODO:Link)를 참고하여 권한을 설정해주세요. 권한을 설정하지 않으면 학습이 실패하거나 학습의 결과물 및 체크포인트 저장에 실패합니다.
