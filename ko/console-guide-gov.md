...
- **상태**: 노트북의 상태가 표시됩니다. 주요 상태는 아래 표를 참고하세요.
    ...
    | CREATE FAILED      | 노트북 생성 중 실패한 상태입니다. 생성이 지속적으로 실패하면 고객지원으로 문의하세요.                                           |
...
!!! danger "주의"
    노트북을 재부팅할 때 사용자가 생성한 가상 환경 및 외부 라이브러리가 초기화될 수 있습니다.
...
- **알고리즘 정보**: 학습하려는 알고리즘에 대한 정보를 입력합니다.
    - **알고리즘 유형**: 알고리즘 유형을 선택합니다.
        - **NHN Cloud 제공 알고리즘**: AI EasyMaker에서 제공하는 알고리즘을 사용합니다. 제공하는 알고리즘에 대한 상세 정보는 [NHN Cloud 제공 알고리즘 가이드](./algorithm-guide-gov/#) 문서를 참고합니다.
            - **알고리즘**: 알고리즘을 선택합니다.
            - **하이퍼파라미터**: 학습에 필요한 하이퍼파라미터 값을 입력합니다. 알고리즘별 하이퍼파라미터에 대한 자세한 정보는 [NHN Cloud 제공 알고리즘 가이드](./algorithm-guide-gov/#) 문서를 참고합니다.
        ...
        - **자체 알고리즘**: 사용자가 작성한 알고리즘을 사용합니다.
            - **알고리즘 경로**
                - **NHN Cloud Object Storage**: 알고리즘이 저장된 NHN Cloud Object Storage의 경로를 입력합니다.<br>
                    ...
                    - NHN Cloud Object Storage를 이용하는 경우 [부록 > 1. NHN Cloud Object Storage에 AI EasyMaker 시스템 계정 권한 추가](#appendix.1.object.storage.account.permission)를 참고하여 권한을 설정하세요. 필요한 권한을 설정하지 않으면 모델 생성에 실패합니다.
            ...
            - **하이퍼파라미터**
                ...
                - 입력된 하이퍼파라미터는 엔트리 포인트가 실행될 때 실행 인자로 입력됩니다. 자세한 활용 방법은 [부록 > 3. 하이퍼파라미터](#appendix.3.hyperparameter)를 참고하세요.

- **이미지**: 학습을 실행해야 하는 환경에 맞게 인스턴스의 이미지를 선택합니다.

- **학습 리소스 정보**
    - **학습 인스턴스 타입**: 학습을 실행할 인스턴스 타입을 선택합니다.
    - **분산 노드 수**: 분산 학습을 수행할 노드 수를 입력합니다. 알고리즘 코드에서 설정을 통해 분산 학습이 가능하게 할 수 있습니다. 자세한 사항은 [부록 > 6. 프레임워크별 분산 학습 설정](#appendix.6.framework.training.settings)을 참고하세요.
    - **torchrun 사용 여부**: Pytorch 프레임워크에서 지원하는 torchrun의 사용 여부를 선택하세요. 자세한 사항은 [부록 > 8. torchrun 사용 방법](#appendix.8.torchrun.usage)을 참고하세요.
...
- **추가 설정**
    ...
    - **로그 관리**: 학습 진행 중 발생하는 로그를 NHN Cloud Log & Crash 서비스에 저장할 수 있습니다.
        - 자세한 내용은 [부록 > 2. NHN Cloud Log & Crash Search 서비스 이용 안내 및 로그 확인](#appendix.2.lncs.service.usage.guide.and.log.inquiry.guide)을 참고하세요.
...
- **상태**: 학습의 상태가 표시됩니다. 주요 상태는 아래 표를 참고하세요.
    ...
    | CREATE FAILED                                | 학습 생성을 실패한 상태입니다. 생성이 지속적으로 실패하면 고객지원으로 문의하세요.                                             |
...
- **작업**
    - **텐서보드 바로가기**: 학습의 통계 정보를 확인할 수 있는 텐서보드가 브라우저 새 창으로 열립니다.<br/>
        텐서보드 로그를 남기는 방법은 [부록 > 5. 텐서보드 활용을 위한 지표 로그 저장](#appendix.5.tensorboard.store.metric.log)을 참고하세요. 텐서보드는 콘솔에 로그인한 사용자만 접속할 수 있습니다.
...
- **알고리즘 정보**: 학습하려는 알고리즘에 대한 정보를 입력합니다.
    - **알고리즘 유형**: 알고리즘 유형을 선택합니다.
        - **NHN Cloud 제공 알고리즘**: AI EasyMaker에서 제공하는 알고리즘을 사용합니다. 제공하는 알고리즘에 대한 상세 정보는 [NHN Cloud 제공 알고리즘 가이드](./algorithm-guide-gov/#) 문서를 참고합니다.
            - **알고리즘**: 알고리즘을 선택합니다.
            - **하이퍼파라미터 스펙**: 하이퍼파라미터 튜닝에 사용할 하이퍼파라미터 값 범위를 입력합니다. 알고리즘별 하이퍼파라미터에 대한 자세한 정보는 [NHN Cloud 제공 알고리즘 가이드](./algorithm-guide-gov/#) 문서를 참고합니다.
...
- **상태**: 하이퍼파라미터 튜닝의 상태가 표시됩니다. 주요 상태는 아래 표를 참고하세요.
    ...
    | CREATE FAILED                                                                  | 하이퍼파라미터 튜닝 생성을 실패한 상태입니다. 생성이 지속적으로 실패하면 고객지원으로 문의하세요.                                               |
...
사전 학습된 거대 언어 모델(Base Model)에 특정 도메인이나 작업에 맞춘 데이터 세트로 추가 학습을 수행하여 모델의 성능을 특화하는 기능입니다. AI EasyMaker에서 제공하는 베이스 모델 중 하나를 선택하고 학습 데이터와 하이퍼파라미터를 입력하여 파인 튜닝을 진행합니다.

<a id="fine.tuning.create"></a>

### 파인 튜닝 생성

베이스 모델과 학습 데이터, 하이퍼파라미터를 설정하여 파인 튜닝을 진행합니다.

- **기본 정보**
    - **이름**: 파인 튜닝 이름을 입력합니다.
    - **설명**: 설명이 필요한 경우에 입력합니다.
    - **실험**: 파인 튜닝이 포함될 실험을 선택합니다. 실험은 연관된 작업을 그룹화합니다. 생성된 실험이 없는 경우 **추가**를 클릭하여 실험을 생성합니다.
- **베이스 모델 정보**
    - **베이스 모델**: 파인 튜닝의 기반이 되는 베이스 모델을 선택합니다. **베이스 모델**을 클릭하면 표시되는 모델 선택 창에서 파인 튜닝용으로 제공되는 베이스 모델 중 하나를 선택합니다.
- **파라미터 정보**
    - **하이퍼파라미터**: 선택한 베이스 모델이 지원하는 하이퍼파라미터 목록이 표시됩니다. 파라미터별 기본값이 채워지며, 수정 가능한 파라미터는 값을 변경할 수 있습니다.
- **인스턴스 정보**
    - **인스턴스 타입**: 파인 튜닝을 실행할 인스턴스 타입을 선택합니다. 선택한 베이스 모델에서 사용 가능한 인스턴스 타입만 표시됩니다.
    - **인스턴스 수**: 파인 튜닝을 수행할 인스턴스 수입니다.
- **입력 데이터**
    - **학습 데이터**: 파인 튜닝에 사용할 학습용 데이터 세트를 입력합니다. 학습 데이터 세트는 최소 1개 이상, 최대 10개까지 설정할 수 있습니다.
        - **데이터 세트 이름**: 데이터 세트 이름을 입력합니다.
        - **데이터 경로**: 데이터가 저장된 NHN Cloud Object Storage 또는 NHN Cloud NAS 경로를 입력합니다.
            - NHN Cloud Object Storage를 이용하는 경우 [부록 > 1. NHN Cloud Object Storage에 AI EasyMaker 시스템 계정 권한 추가](#appendix.1.object.storage.account.permission)를 참고하여 권한을 설정하세요. 필요한 권한을 설정하지 않으면 파인 튜닝에 실패합니다.
        - **데이터 형식**: 학습 데이터의 형식입니다. (`chat_template` 고정) 데이터 세트 형식에 대한 자세한 내용은 [부록 > 12. 파인 튜닝 데이터 세트 형식](#appendix.12.fine.tuning.dataset.format)을 참고하세요.
    - **검증 데이터**: 파인 튜닝에 사용할 검증용 데이터 세트를 입력합니다. 검증 데이터 세트는 최대 10개까지 설정할 수 있으며, 입력 항목은 학습 데이터와 동일합니다.
    - **검증 데이터 비율**: 별도의 검증 데이터 세트 없이 학습 데이터의 일부를 검증용으로 사용하려는 경우, 검증용으로 사용할 비율(%)을 입력합니다.
        - 검증 데이터 세트가 입력된 경우에는 0으로 고정되며 수정할 수 없습니다.
- **출력 데이터**
    - **모델 업로드 경로**: 파인 튜닝이 완료된 모델을 저장할 경로를 입력합니다.
        - NHN Cloud Object Storage 또는 NHN Cloud NAS 경로를 입력합니다.
- **추가 설정**
    - **데이터 스토리지 크기**: 파인 튜닝을 실행할 인스턴스의 데이터 스토리지 크기를 입력합니다.
        - NHN Cloud Object Storage를 사용하는 경우에만 사용됩니다. 파인 튜닝에 필요한 데이터가 모두 저장될 수 있도록 충분한 크기로 지정해야 합니다.
    - **최대 진행 시간**: 파인 튜닝이 완료될 때까지 최대 대기 시간을 지정합니다. 최대 대기 시간이 초과한 파인 튜닝은 종료 처리됩니다.
    - **로그 관리**: 파인 튜닝 진행 중 발생하는 로그를 NHN Cloud Log & Crash 서비스에 저장할 수 있습니다.
        - 자세한 내용은 [부록 > 2. NHN Cloud Log & Crash Search 서비스 이용 안내 및 로그 확인](#appendix.2.lncs.service.usage.guide.and.log.inquiry.guide)을 참고하세요.
...
- **베이스 모델**: 파인 튜닝에 사용된 베이스 모델 이름이 표시됩니다.
- **소요 시간**: 파인 튜닝이 진행된 시간이 표시됩니다.
- **상태**: 파인 튜닝의 상태가 표시됩니다. 주요 상태는 아래 표를 참고하세요.

    | 상태                                                          | 설명                                                                                                                                  |
    | ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
    | CREATE IN PROGRESS                                            | 파인 튜닝에 필요한 자원을 생성 중인 상태입니다.                                                                                       |
    | RUNNING                                                       | 파인 튜닝이 진행 중인 상태입니다.                                                                                                     |
    | STOPPED                                                       | 파인 튜닝이 사용자의 요청으로 중지된 상태입니다.                                                                                      |
    | COMPLETE                                                      | 파인 튜닝이 정상적으로 완료된 상태입니다.                                                                                             |
    | STOP IN PROGRESS                                             | 파인 튜닝이 중지 중인 상태입니다.                                                                                                     |
    | FAIL FINE TUNING                                              | 파인 튜닝 진행 중 실패한 상태입니다. 자세한 실패 정보는 로그 관리가 활성화된 경우, Log & Crash Search 로그를 통해 확인할 수 있습니다. |
    | CREATE FAILED                                                 | 파인 튜닝 생성을 실패한 상태입니다. 생성이 지속적으로 실패하면 고객지원으로 문의하세요.                                              |
    | STOP FAILED                                                   | 파인 튜닝 중지를 실패한 상태입니다.                                                                                                   |
    | FAIL FINE TUNING IN PROGRESS, COMPLETE IN PROGRESS, STOP IN PROGRESS | 파인 튜닝에 사용된 리소스를 정리 중인 상태입니다.                                                                              |

- **작업**
    - **텐서보드 바로가기**: 파인 튜닝의 통계 정보를 확인할 수 있는 텐서보드가 브라우저 새 창으로 열립니다. 텐서보드는 콘솔에 로그인한 사용자만 접속할 수 있습니다.
    - **중지**: 진행 중인 파인 튜닝을 중지할 수 있습니다.

- **하이퍼파라미터**: 파인 튜닝을 선택하면 표시되는 상세 화면의 **하이퍼파라미터** 탭에서 파인 튜닝에 설정한 하이퍼파라미터 값을 확인할 수 있습니다.

- **모니터링**: 파인 튜닝을 선택하면 표시되는 상세 화면의 **모니터링** 탭에서 모니터링 대상 인스턴스 목록과 기본 지표 차트를 확인할 수 있습니다.
...
1. 복사하려는 파인 튜닝을 선택합니다. 파인 튜닝이 한 개만 선택된 경우에 사용할 수 있습니다.
2. **복사**를 클릭합니다.
3. 기존 파인 튜닝과 동일한 설정으로 파인 튜닝 생성 화면이 표시됩니다.
...
1. 모델로 생성하려는 파인 튜닝을 선택합니다.
2. **모델 생성**을 클릭합니다. 완료(COMPLETE) 상태의 파인 튜닝만 모델로 생성할 수 있습니다.
3. 모델 생성 페이지로 이동됩니다. 내용을 확인 후 **모델 생성**을 클릭하여 모델을 생성합니다. 모델 생성에 대한 자세한 내용은 [모델](#model) 문서를 참고하세요.
...
1. 삭제하려는 파인 튜닝을 선택합니다. 생성 실패(CREATE FAILED), 실패(FAIL FINE TUNING), 완료(COMPLETE), 중지됨(STOPPED) 상태의 파인 튜닝만 삭제할 수 있습니다.
2. **삭제**를 클릭합니다.
3. 요청된 삭제 작업은 취소할 수 없습니다. 계속 진행하려면 **확인**을 클릭합니다.
...
- **모델 정보**: 모델의 아티팩트가 저장된 저장소를 입력합니다.
    - **모델 아티팩트**: 모델 아티팩트가 저장된 저장소를 선택합니다.
        - **NHN Cloud Object Storage**: 모델 아티팩트가 저장된 Object Storage 경로를 입력합니다.
            - `obs://{Object Storage API 엔드포인트}/{containerName}/{path}` 형식으로 디렉터리 경로를 입력합니다.
            - NHN Cloud Object Storage를 이용하는 경우 [부록 > 1. NHN Cloud Object Storage에 AI EasyMaker 시스템 계정 권한 추가](#appendix.1.object.storage.account.permission)를 참고하여 권한을 설정하세요. 권한을 설정하지 않으면 모델의 아티팩트에 접근이 불가하여 모델 생성에 실패합니다.
...
!!! tip "알아두기"
    ...
    자세한 내용은 [부록 > 11. 프레임워크별 서빙 참고 사항](#appendix.11.framework.note)을 참고하세요.
...
- **상태**: 모델의 상태가 표시됩니다. 주요 상태는 아래 표를 참고하세요.
    ...
    | CREATE FAILED      | 모델 생성을 실패한 상태입니다. 생성이 지속적으로 실패하면 고객지원으로 문의하세요. |
...
- **API Gateway 서비스 활성화**
    ...
    - API Gateway 서비스에 대한 자세한 내용과 요금은 다음에서 확인할 수 있습니다.
        - [API Gateway 서비스 안내](https://docs.gov-nhncloud.com/ko/Application%20Service/API%20Gateway/ko/overview/)
        - [API Gateway 이용 요금](https://www.gov-nhncloud.com/kr/pricing/by-service?c=Application%20Service&s=API%20Gateway)
...
- **스테이지 정보**: 엔드포인트에 배포할 모델 아티팩트의 정보를 입력합니다. 같은 모델을 여러 스테이지 리소스로 배포하면 요청이 분산되어 처리됩니다.
    - **모델**: 엔드포인트에 배포하려는 모델을 선택합니다. 모델을 생성하지 않은 경우 모델을 먼저 생성합니다. 모델 프레임워크별 서빙 참고 사항은 [부록 > 11. 프레임워크별 서빙 참고 사항](#appendix.11.framework.note)을 참고하세요.
    ...
    - **리소스 정보**: 실제 사용하는 리소스를 확인할 수 있습니다. 입력한 모델의 할당량에 따라 리소스 실 사용량을 각 모델에 할당합니다. 자세한 내용은 [부록 > 9. 리소스 정보](#appendix.9.resource.info)를 참고하세요.

!!! tip "알아두기"
    AI EasyMaker 서비스는 OIP(open inference protocol) 스펙을 기반으로 한 엔드포인트를 제공합니다. 엔드포인트 API 스펙 명세는 [부록 > 10. 엔드포인트 API 스펙 명세](#appendix.10.endpoint.api.specification)를 참고하세요.
...
- **상태**: 엔드포인트의 상태입니다. 주요 상태는 아래 표를 참고하세요.
    ...
    | CREATE FAILED      | 엔드포인트 생성에 실패한 상태입니다.<br/>엔드포인트를 삭제 후 다시 생성해야 합니다. 생성 실패 상태가 반복되면 고객지원으로 문의하세요.   |
...
!!! danger "주의"
    ...
    4. API Gateway의 스테이지 설정에서 API Gateway 리소스 경로에 설정된 **백엔드 엔드포인트 URL 재정의**를 비활성화하거나 URL을 변경하지 않도록 합니다. 변경하면 엔드포인트의 추론 API 호출에 실패할 수 있습니다.
       ...
       자세한 API Gateway 사용에 대한 내용은 [API Gateway 콘솔 가이드](https://docs.gov-nhncloud.com/ko/Application%20Service/API%20Gateway/ko/console-guide-gov/)를 참고하세요.
...
- **API Gateway 리소스 경로**: 스테이지에 배포된 모델의 추론 URL입니다. 표시된 URL로 추론을 요청할 수 있습니다. 자세한 내용은 [부록 > 10. 엔드포인트 API 스펙 명세](#appendix.10.endpoint.api.specification)를 확인하세요.
...
- **모델 정보**
    ...
    - **리소스 정보**: 모델에서 실제 사용하는 리소스를 확인할 수 있습니다. 입력한 파드 수에 따라 실사용량을 분할하여 각 파드에 할당합니다. 자세한 내용은 [부록 > 9. 리소스 정보](#appendix.9.resource.info)를 참고하세요.
- **입력 데이터**
...
- **상태**: 배치 추론의 상태가 표시됩니다. 주요 상태는 아래 표를 참고하세요.
    ...
    | CREATE FAILED                                          | 배치 추론 생성을 실패한 상태입니다. 생성이 지속적으로 실패하면 고객지원으로 문의하세요.                                              |
...
2. 개인 이미지 빌드와 컨테이너 레지스트리 Push
    Dockerfile로 이미지를 빌드하고 NCR 레지스트리에 이미지를 저장(Push)합니다.
        ...
        docker tag custom-training:v1 example-kr1-registry.container.gov-nhncloud.com/registry/custom-training:v1
        docker push example-kr1-registry.container.gov-nhncloud.com/registry/custom-training:v1
...
!!! tip "알아두기"
    ...
    파이프라인 코드의 로그는 [SDK의 Log 전송 기능](./sdk-guide-gov/#feature.lncs.log.send)을 이용하여 Log & Crash Search로 전송하여 확인하세요.
...
파이프라인 일정 생성에 설정할 수 있는 정보 중 아래 항목 외 정보는 [파이프라인 실행 생성](#pipeline.run.create)을 참고하세요.
...
!!! tip "알아두기"
    수집에서 사용할 수 있는 파일의 포맷과 크기, 개수가 제한될 수 있습니다. 자세한 내용은 [수집 동기화](#rag.ingestion.sync)를 참고하세요.

!!! danger "주의"
    PostgreSQL Instance 사용 시 포트를 `15432`로 설정하세요.
    인스턴스를 생성하는 방법은 [PostgreSQL Instance](https://docs.gov-nhncloud.com/ko/Compute/Instance/ko/component-guide-gov/#postgresql-instance)를 참고하세요.
...
| CREATE FAILED | RAG 생성에 실패한 상태입니다.<br/>RAG를 삭제한 후 다시 생성하세요. 생성 실패가 반복되면 고객지원으로 문의하세요. |
| UPDATE FAILED | RAG에서 수집이 실패한 상태입니다.<br/>**수집 동기화**를 다시 시도하세요. 업데이트 실패가 반복되면 고객지원으로 문의하세요. |
| DELETE FAILED | RAG 삭제에 실패한 상태입니다.<br/>삭제를 다시 시도하세요. 삭제 실패가 반복되면 고객지원으로 문의하세요. |
...
- **Log & Crash Search 서비스 이용 및 요금 안내**
    - Log & Crash Search 서비스에 대한 자세한 내용과 요금은 다음에서 확인할 수 있습니다.
        - [Log & Crash Search 서비스 안내](https://docs.gov-nhncloud.com/ko/Data%20&%20Analytics/Log%20&%20Crash%20Search/ko/gov-overview/)
...
3. Log & Crash Search 서비스의 자세한 이용 방법은 [Log & Crash Search 서비스의 콘솔 가이드](https://docs.gov-nhncloud.com/ko/Data%20&%20Analytics/Log%20&%20Crash%20Search/ko/gov-console-guide/)를 참고하세요.
...
|------------------|------|------------------------------------------------------------|
| 모델 목록            | GET  | /v1/models                                                 |
| 모델 Ready         | GET  | /v1/models/{model_name}                                    |
| 추론               | POST | /v1/models/{model_name}/predict                            |
| 설명               | POST | /v1/models/{model_name}/explain                            |
| 서버 정보            | GET  | /v2                                                        |
| 서버 Live          | GET  | /v2/health/live                                            |
| 서버 Ready         | GET  | /v2/health/ready                                           |
...
1. Hugging Face 모델을 다운로드합니다.
    ...
    - 미세 조정이 필요한 경우 [Hugging Face 미세 조정 가이드](https://huggingface.co/docs/transformers/main/ko/training)에 따라 자체 코드를 작성해서 학습할 수 있습니다.
        - AI EasyMaker 학습에 대한 자세한 내용은 [학습](#training)을 확인하세요.
...
4. 이후의 과정은 [모델 생성](#model.create)과 [엔드포인트 생성](#endpoint.create) 가이드를 확인하세요.
...
- 파일은 JSONL(JSON Lines) 형식으로, 한 줄에 하나의 대화 샘플(JSON 객체)을 작성합니다.
- 각 샘플은 `messages` 배열로 구성되며, 배열의 각 항목은 `role`과 `content`로 이루어집니다.
    - `role`: 메시지를 발화하는 주체입니다. `system`, `user`, `assistant`를 사용합니다.
        - `system`: 모델의 역할이나 지침을 정의합니다(선택).
        - `user`: 사용자 입력입니다.
        - `assistant`: 모델이 생성해야 하는 정답 응답입니다.
    - `content`: 해당 역할의 텍스트 내용입니다.
...
!!! tip "알아두기"
    - 파일 확장자는 `.jsonl`을 사용하며, 한 줄에 하나의 JSON 객체만 작성합니다.
    - `assistant` 메시지가 모델이 학습하는 정답입니다.
    - 검증 데이터도 학습 데이터와 동일한 형식으로 작성합니다.
